<template>
  <div class="q-pa-xl">
    ...
    <q-form @submit.prevent="guardarProducto" @reset="close" ref="myForm">
      ...
      <q-input  class="col-xs-12 col-md-4 q-py-md q-px-sm" v-model="producto.codigo" label="Nombre *"  dense
        color="primary"
                  counter
                  maxlength="50"
                  lazy-rules
      ...
      <q-select
                    ...
                    >
                        ...
                        <q-input  step="any" v-model.number="producto.precioCosto" label="Precio Costo" type="number" outlined dense class="col-xs-12 col-md-3 q-py-md q-px-sm"  />
        <q-select
          ...
          <q-input  step="any" v-model.number="producto.precioVenta" label="Precio Venta" type="number" outlined dense class="col-xs-12 col-md-3 q-py-md q-px-sm"  />
     ...
     <q-select
      ...
      <q-input autogrow class="col-xs-12 col-md-12 q-py-md q-px-sm" v-model="producto.descripcion" label="Descripción *"  dense
         color="primary"
                  counter
                  lazy-rules
                  :rules="[
...
<div>
      ...
      <q-checkbox @click="funcionalidadSinVariantes()" v-model="producto.sinVariantes" label="Producto sin variante" color="primary" />
</div>
      ...
      <div class="text-h6 q-mb-md col-xs-12 col-sm-12 col-md-12" v-if="!producto.sinVariantes">Variantes
      ...
      <div v-for="(variant, index) in producto.variants" :key="index" class="col-xs-12 q-pa-sm q-mb-md bordered">
       ...
       <q-input v-if="!producto.sinVariantes" v-model="variant.skuVariante" label="SKU Variante *"  dense class="col-xs-12 col-md-3  q-px-sm" lazy-rules outlined
                  :rules="[(val) =>
                      (val && val.length > 0) || 'Debe insertar un SKU']" />

<q-input  dense class="col-xs-12 col-md-3  q-px-sm" lazy-rules outlined v-model="variant.talla" label="Talla"
        color="primary"
                  counter
                  maxlength="20"
                  :rules="[]"
...
>
  ...
  <div v-else class="q-pa-sm text-grey">No hay imágenes para mostrar.</div>
      ...
      <div class="row q-mt-sm" v-if="!producto.sinVariantes">
        <q-btn
        color="negative"
        icon="delete"
...
const obtenerElementoPorId = async (id) => {
  ...
  // Separar el SKU del producto del SKU de la variante
  if (objeto.sku && producto.value.variants.length > 0) {
    producto.value.variants.forEach(variant => {
      if (variant.skuVariante && variant.skuVariante.startsWith(objeto.sku)) {
        // Quitamos exactamente la parte del SKU del producto desde el inicio (ej: "PROD001V1" -> "V1")
        variant.skuVariante = variant.skuVariante.substring(objeto.sku.length)
      }
    })
  }
...
function eliminarTodasVariante() {
     producto.value.variants.splice(0, producto.value.variants.length);
  //producto.value.variants = [];
  // or producto.value.variants.splice(0, producto.value.variants.length);
}
...
async function guardarProducto() {
  ...
  try {
    ...
    if(!producto.value?.sinVariantes){
    if(producto.value?.variants.length===0){
      ...
      return
        ...
        if(producto.value?.variants.length===0){
      Error("Debe agregar al menos una foto")
      dialogLoad.value = false
      return
    }
    ...
    // Validar que al menos una variante tenga fotos
    const algunaVarianteConFoto = producto.value.variants.some(variant => {
      return variant.fotos && variant.fotos.length > 0
    })

    if (!algunaVarianteConFoto) {
      Error("Al menos una variante debe tener una foto")
      dialogLoad.value = false
      return
    }
    ...
    formData.append("Descripcion", producto.value?.descripcion || '')
    formData.append("EsActivo", producto.value?.esActivo ?? true)
    formData.append("SinVariantes", producto.value?.sinVariantes ?? false)
    formData.append("SKU", producto.value?.sku || '')
    formData.append("PrecioCosto", producto.value?.precioCosto || 0)
    formData.append("PrecioVenta", producto.value?.precioVenta || 0)
    formData.append("MonedaCostoId", producto.value?.monedaCostoId || '')
    ...
    if (Array.isArray(producto.value?.variants)) {
      producto.value.variants.forEach((variant, index) => {
        formData.append(`ProductoVariantes[${index}].ProductoId`, variant.id || '')
        // El backend se encarga de unir el SKU del producto con el de la variante
        if(!producto.value?.sinVariantes){
        formData.append(`ProductoVariantes[${index}].SkuVariante`, variant.skuVariante || '')
        }
        else{
        formData.append(`ProductoVariantes[${index}].SkuVariante`, producto.value?.sku || '')
        }
        formData.append(`ProductoVariantes[${index}].Talla`, variant.talla || '')
        formData.append(`ProductoVariantes[${index}].Color`, variant.color || '')
        formData.append(`ProductoVariantes[${index}].Stock`, variant.stock || 0)
...
function funcionalidadSinVariantes(){
        eliminarTodasVariante()
...
/*
watch(() => producto.value.sinVariantes, (val) => {
